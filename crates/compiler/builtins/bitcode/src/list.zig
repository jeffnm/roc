...
pub fn listReserve(
    ...
    if ((update_mode == .InPlace or list.isUnique()) and list.getCapacity() >= list.len() + spare) {
        ...
        var output = list.reallocate(alignment, old_length + spare, element_width, elements_refcounted, inc);
...
pub fn listReplace(
    ...
    // so we don't do a bounds check here. Hence, the list is also non-empty,
    // because inserting into an empty list is always out of bounds
    return listReplaceInPlaceHelp(list.makeUnique(alignment, element_width, elements_refcounted, inc, dec), index, element, element_width, out_element);
}
